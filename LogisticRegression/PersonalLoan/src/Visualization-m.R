...
date_decile <- function(x){
  
  deciles <- vector(length=10)
  deciles <- quantile(as.integer(x), 
...
fn_biz_viz_date <- function(df, target, var)
{
  names(var)
  tmp <- df[, c(var , target)]
  colnames(tmp)[1] = "Xvar"
  ...
  colnames(tmp)
  #ifelse(class(tmp$Xvar) == "Date", tmp$deciles <- date_decile(tmp$Xvar),tmp$deciles <- decile(tmp$Xvar))
  tmp$deciles <- date_decile(tmp$Xvar)
  
  
  tmp_DT = data.table(tmp)
  
  RRate <- tmp_DT[, list(
    min_ = min(Xvar), max_ = max(Xvar), avg_ = mean.Date(Xvar),
